...
#         dbname='dbfunc', 
#         user='postgres',
#         password='886744@Jo_BD', 
#         sslmode='prefer', 
#         connect_timeout=10
#     )
#     # Aquisição de um cursor
#     cursor = conexao.cursor() # Execução comando: SELECT..CRETAE.. 
#     cursor.execute("SELECT * FROM pacientes") 
#     registros = cursor.fetchall() 
#     for registro in registros:
#         print(registro) # Efetivação do comando
#     conexao.commit() # Fechamente das conexões
#     cursor.close() 
#     conexao.close() 
# except psycopg2.Error as e:
#     print(f"Erro ao conectar ao banco de dados: {e}")  # Erro ao conectar ao banco de dados 
...
class AppBD:
       ...
       def createDataTable(self, db, table_1, table_2, table_3):
            try:
                ...
                print(f"Tabela criada '{table_2}' com sucesso ou esta tabela já existe no banco da dados!")
                print(f"Tabela criada '{table_3}' com sucesso ou esta tabela já existe no banco da dados!")
...
aluno = Alunos(22, "Jessica Elen", 24, "Engenharia Civil", "jessica.elen@gmail.com", "(45) 123456789", "F")
# print(aluno.matricula, aluno.nome, aluno.idade, aluno.curso, aluno.email, aluno.telefone, aluno.sexo, aluno.id_notas)

# Criação da conexão com o banco de dados MySQL e da tabela Alunos:
...
valores = [str(aluno.matricula), str(aluno.nome), str(aluno.idade), str(aluno.curso), str(aluno.email), str(aluno.telefone), str(aluno.sexo)]
...
aluno_2 = Alunos(35, "Bruna Marcs", 21, "Engenharia de Software", "bruna.marcs@gmail.com", "(44) 9989454556", "F")

colunas = ["matricula", "nome", "idade", "curso", "email", "telefone", "sexo"]
valores = [str(aluno_2.matricula), str(aluno_2.nome), str(aluno_2.idade), str(aluno_2.curso), str(aluno_2.email), str(aluno_2.telefone), str(aluno_2.sexo)]
...
aluno_3 = Alunos(65, "Otaviano Luc", 21, "ADS", "otav_luc23.marcs@gmail.com", "(44) 99894555786", "M")

colunas = ["matricula", "nome", "idade", "curso", "email", "telefone", "sexo"]
valores = [str(aluno_3.matricula), str(aluno_3.nome), str(aluno_3.idade), str(aluno_3.curso), str(aluno_3.email), str(aluno_3.telefone), str(aluno_3.sexo)]
...
class Disciplinas(AppBD):    
    def __init__(self, id_disciplina, nome_disciplina, carga_horaria, descricao, valor, nota_corte):
        self.id_disciplina = id_disciplina
        self.nome_disciplina = nome_disciplina
        self.carga_horaria = carga_horaria
        self.descricao = descricao
        self.valor = valor
        self.nota_corte = nota_corte # Nota suficiente para o aluno passar na disciplina
        
# Objeto da classe Disciplinas:   
disciplina = Disciplinas(1, "Matemática", 40, "Equação do Segundo Grau", 90.87, 6.0)
...
# Inserindo dados no banco de dados
colunas = ["id_disciplina", "nome_disciplina", "nota_corte", "valor", "descricao", "carga_horaria"]
                                
valores = [str(disciplina.id_disciplina), str(disciplina.nome_disciplina), str(disciplina.nota_corte), str(disciplina.valor), str(disciplina.descricao), str(disciplina.carga_horaria)]
...
# disciplina.atualizar_dados("disciplinas", colunas, valores)

# Exclusão dos dados da tabela Aluno no banco de dados MySQL
# disciplina.excluir_dados("disciplinas", "nome_disciplina", "Matemática")

class Notas(AppBD):
    def __init__(self, id, nota_1, nota_2, nota_3, id_aluno, id_disciplina, media_arit = 0, soma = 0, status = ""):
        self.id = id
        self.nota_1 = nota_1
        self.nota_2 = nota_2
        self.nota_3 = nota_3
    ...
    def gerarRelatorioNotas_csv(self):
        ...
        with open('notas.csv', 'w', newline='') as csvfile:
            fieldnames = ['id', 'nota_1', 'nota_2', 'nota_3', 'media_arit', 'soma', 'status']
...
colunas = ["id_nota", "nota_1", "nota_2", "nota_3", "media", "id_aluno", "id_disciplina", "status"]
                                
valores = [str(notas.id), str(notas.nota_1), str(notas.nota_2), str(notas.nota_3), str(notas.media_arit), str(notas.id_aluno), str(notas.id_disciplina), str(notas.status)]
...
notas.atualizar_dados("notas", colunas, valores, "id_nota", 1) # Atualiza os dados da tabela professor no banco de dados MySQL

# Exclusão dos dados da tabela Aluno no banco de dados MySQL
notas.excluir_dados("notas", "id_nota", 1)

# class Professor(AppBD):
#     def __init__(self, matricula, nome, disciplinas, cursos, email, telefone, graduacao, sexo):
#         self.matricula = matricula
#         self.nome = nome
#         self.disciplinas = disciplinas # Um professor pode ministrar 1 ou mais disciplinas 
#         self.cursos = cursos # Um professor pode ministrar 1 ou mais cursos
#         self.email = email
#         self.telefone = telefone
#         self.graduacao = graduacao 
#         self.sexo = sexo 
        
# prof = Professor(20230, "John Victor Parreiras Soares", "Cibersegurança", "Sistemas de Informação", "johnvps@gmail.com", "123456789", "Segurança da Informação", "M")
# # print(prof.matricula, prof.nome, prof.disciplinas, prof.cursos, prof.email, prof.telefone, prof.graduacao)         

# # Inserir dados na tabela Alunos através do objeto da classe Alunos:
# colunas = ["matricula", "nome", "disciplinas", "cursos", "email", "telefone", "graduacao", "sexo"]
                                
# valores = [str(prof.matricula), str(prof.nome), str(prof.disciplinas), str(prof.cursos), str(prof.email), str(prof.telefone), str(prof.graduacao), str(prof.sexo)]

# prof.inserir_dados("professores", colunas, valores)
# # Exibição dos registros da tabela professor no banco de dados MySQL
# print(prof.selecionar_dados("professores"))

# prof.atualizar_dados("professores", colunas, valores)

# # Exclusão dos dados da tabela Aluno no banco de dados MySQL
# prof.excluir_dados("professores", "nome", "John Victor Parreiras Soares")

# class Cursos(AppBD):
#     def __init__(self, id_curso, nome, carga_horaria, descricao, valor, id_aluno):
...
# # Objeto da classe Cursos:   
# curso = Cursos(1, "Engenharia", 48, "Desenvolvimento de software", 1000, 1)
# # print(curso.id_curso, curso.nome, curso.carga_horaria, curso.descricao, curso.valor, curso.id_aluno)

# colunas = ["id_curso", "nome_curso", "carga_horaria", "descricao", "valor", "id_aluno"]

# valores = [str(curso.id_curso), str(curso.nome), str(curso.carga_horaria), str(curso.descricao), str(curso.valor), str(curso.id_aluno)]

# curso.inserir_dados("cursos", colunas, valores)
# # Exibição dos registros da tabela professor no banco de dados MySQL
# print(curso.selecionar_dados("cursos"))

# curso.atualizar_dados("cursos", colunas, valores)

# # Exclusão dos dados da tabela Aluno no banco de dados MySQL
# curso.excluir_dados("cursos", "id_curso", 2)
    ...
    while True:
        ...
        if escolha == "1":
            ...
            if escolha_tabela == "1":
                    ...
                    disciplina = Disciplinas(id_disciplina, nome_disciplina, carga_horaria, descricao, valor, nota_corte)
                    colunas = ["id_disciplina", "nome_disciplina", "nota_corte", "valor", "descricao", "carga_horaria"]
                    valores = [str(disciplina.id_disciplina), str(disciplina.nome_disciplina), str(disciplina.nota_corte), str(disciplina.valor), str(disciplina.descricao), str(disciplina.carga_horaria)]
                    disciplina.inserir_dados("disciplinas", colunas, valores)
                    print(disciplina.selecionar_dados("disciplinas"))
                    ...
                    id_disciplina = int(input("ID da disciplina: "))
                    nota = Notas(id_nota, nota_1, nota_2, nota_3, id_aluno, id_disciplina, 0, 0, "")
                    nota.retornarMediaArit()
                    nota.retornarSoma()
                    nota.retornarStatus()
                    colunas = ["id_nota", "nota_1", "nota_2", "nota_3", "media", "id_aluno", "id_disciplina", "status"]
                    valores = [str(nota.id), str(nota.nota_1), str(nota.nota_2), str(nota.nota_3), str(nota.media_arit), str(nota.id_aluno), str(nota.id_disciplina), str(nota.status)]
                ...
                if escolha_tabela == "1":   
                    ...
                    if escolha_csv == "1":
                        try:
                            aluno.selecionar_dados("alunos").to_csv("alunos.csv", index=False)
                            disciplina.selecionar_dados("disciplinas").to_csv("disciplinas.csv", index=False)
                            ...
                            df_disciplinas = pd.DataFrame(disciplina.selecionar_dados("disciplinas"))
            ...
            if escolha_tabela == "1":
                ...
                telefone = input("Telefone do aluno: ")
                genero = input("Gênero do aluno: ")
                aluno = Alunos(matricula_atualizada, nome_aluno, idade_aluno, curso, email, telefone, genero)
                colunas = ["matricula", "nome", "idade", "curso", "email", "telefone", "sexo"]
                valores = [str(aluno.matricula), str(aluno.nome), str(aluno.idade), str(aluno.curso), str(aluno.email), str(aluno.telefone), str(aluno.sexo)]
                aluno.atualizar_dados("aluno", colunas, valores, "matricula", aluno.matricula)
            elif escolha_tabela == "2":
                    ...
                    valor = float(input("Valor da disciplina (R$): "))
                    nota_corte = float(input("Nota de corte da disciplina: "))
                    disciplina = Disciplinas(id_disciplina_atualizada, nome_disciplina, carga_horaria, descricao, valor, nota_corte)
                    colunas = ["id_disciplina", "nome_disciplina", "nota_corte", "valor", "descricao", "carga_horaria"]
                    valores = [str(disciplina.id_disciplina), str(disciplina.nome_disciplina), str(disciplina.nota_corte), str(disciplina.valor), str(disciplina.descricao), str(disciplina.carga_horaria)]
                    disciplina.atualizar_dados("disciplinas", colunas, valores, "id_disciplina", id_disciplina)
                    print(disciplina.selecionar_dados("disciplinas"))
            elif escolha_tabela == "3":
                    ...
                    id_aluno = int(input("ID do aluno: "))
                    id_disciplina = int(input("ID da disciplina: "))
                    nota = Notas(id_nota_atualizada, nota_1, nota_2, nota_3, id_aluno, id_disciplina, 0, 0, "")
                    nota.retornarMediaArit()
                    nota.retornarSoma()
                    nota.retornarStatus()
                    colunas = ["id_nota", "nota_1", "nota_2", "nota_3", "media", "id_aluno", "id_disciplina", "status"]
